...
2. Откройте `run_User_Creator.bat` для редактирования (блокнот), измените  строку `C:\Users\geg\Documents\Script's\User_Creator\User_Creator.ps1` на путь до `User_Creator.ps1` файла
3. Запустите `run_User_Creator.bat` (**от имени пользователя с правами записи в директорию!**).
4. В появившемся запросе `Использовать данные из AD по логину (Y) или по CSV (N)? (y/n)` необходимо определить метод получения **GUID пользователей** - используя AD или использовать `person_guid` и автоматическую генерацию GUID из файла `.csv`
5. Логика обработки при выборе `Y`:
...
1. Логика обработки при выборе `N`:
...
1. Скрипт создаст для каждого исходного CSV два XML-файла (`sysconfig.xml`, `energy.xml`).
2. Если в строках исходного CSV не было указан `person_guid`, он впишется обратно в CSV автоматически.
3.  Итоговый CSV сохранится **в кодировке Windows-1251** для идеальной совместимости с Excel.

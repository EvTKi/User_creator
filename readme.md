...
*   **Гибкая настройка**: Конфигурация через `./config/config.json` и `./config/logging_config.json`.
...
| ------------------------- | --------------- | ----------------------------------------------------------------------------------------- |
| person\_guid              | ❌               | GUID пользователя (может быть пустым — заполнится скриптом)                               |
...
| department                | ❌               | GUID подразделения пользователей                                                          |
| organisation              | ❌               | GUID организации пользователей                                                            |
| OperationalAuthorities    | ❌               | Список GUID полномочий через ! (energy.xml — массив блоков OperationalAuthorities)        |
| electrical\_safety\_level | ❌               | GUID уровня электробезопасности (опционально, energy.xml — ElectricalSafetyLevel)         |
| parent\_energy            | ✅               | GUID родителя для energy.xml                                                              |
| parent\_sysconfig         | ✅               | GUID родителя для sysconfig.xml                                                           |
...
1. Убедитесь, что файлы `./config/config.json` и `./config/logging_config.json` находятся в корневой директории проекта.

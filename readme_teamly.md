...
2.  Убедитесь, что файлы `config.json` и `logging_config.json` находятся в папке `./config/`.
...
| department                | ❌               | GUID подразделения пользователей (опционально, попадёт в energy.xml как Department)       |
| organisation              | ❌               | GUID организации пользователей (опционально, попадёт в energy.xml как Organisation)       |

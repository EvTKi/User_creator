...
# Настройка логгера для UI
ui_logger = logging.getLogger("UserCreatorUI")
ui_logger.setLevel(logging.DEBUG)

# Создаем обработчик для логов UI (можно сохранять в отдельный файл)
# ui_handler = logging.FileHandler('ui.log', encoding='utf-8')
    ...
    def run(self):
        ...
        try:
            ...
            if self.input_dir and os.path.exists(self.input_dir):
                ...
                self.logger.info(f"Рабочая директория изменена на: {os.getcwd()}")
            else:
                self.logger.warning(f"Указанная директория не существует: {self.input_dir}. Используется текущая.")
            ...
            if self.mode == 'y' and AD_ENABLED:
                ...
                self.logger.info(f"Попытка подключения к AD: сервер={CONFIG['ad']['domain_controller']}, пользователь={CONFIG['ad']['user']}")
                ...
                self.logger.info("Выбран режим работы без Active Directory (ручной ввод GUID)")
                ...
                self.logger.info(f"Используется введенный GUID домена: {ad_guid}")
            ...
            # Обработка файлов
            for i, csv_file in enumerate(csv_files):
                self.logger.info(f"Начало обработки файла {i+1}/{total_files}: {csv_file}")
                self.log_signal.emit(f"📄 Обработка файла ({i+1}/{total_files}): {csv_file}")
                ...
                try:
                    ...
                    self.logger.debug(f"Определение кодировки файла: {file_path}")
                    ...
                    self.logger.info(f"Прочитано {len(rows)} строк из файла {csv_file}")
                    
                    updated_rows = []
                    users_data = []
                    
                    self.logger.debug(f"Начало обработки строк из файла {csv_file}")
                    ...
                    self.logger.info(f"Обработано {len(users_data)} записей из файла {csv_file}")
                    self.log_signal.emit(f"  ✅ Обработано записей: {len(users_data)}")
                    ...
                    self.logger.info(f"Обработка файла {csv_file} завершена успешно")
            ...
            if self.mode == 'y' and AD_ENABLED and not_found_in_ad:
                self.logger.info(f"Сохранение списка пользователей не найденных в AD: {len(not_found_in_ad)} записей")
                self.log_signal.emit(f"💾 Сохранение списка пользователей не найденных в AD ({len(not_found_in_ad)} записей)...")
                ...
                try:
                    ...
                    self.logger.info(f"Список пользователей не найденных в AD успешно сохранен в: {NOT_IN_AD_CSV}")
            ...
            self.logger.info(f"Возвращение в исходную директорию: {original_dir}")
...
class UserCreatorWindow(QMainWindow):
    ...
    def run_processing(self):
        ...
        self.logger.info(f"Параметры запуска: режим={mode}, директория={input_dir}")
        ...
        if not input_dir or not os.path.exists(input_dir):
            self.logger.warning(f"Указана несуществующая директория: {input_dir}")
    ...
    def load_settings(self):
        ...
        self.logger.info(f"Настройки загружены: режим={mode}, директория={input_dir}")
    ...
    def closeEvent(self, event):
        ...
        if self.worker and self.worker.isRunning():
            self.logger.warning("Попытка закрытия приложения во время выполнения задачи")
